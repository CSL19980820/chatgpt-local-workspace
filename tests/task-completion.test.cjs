...
test('completion checks, failure receipts, explicit blockers and resumable UI', {timeout:160000}, async t=>{
  ...
  try{
    ...
    await page.goto(base);await page.locator('#task-details').waitFor();assert.equal(await page.locator('#plan-body').isVisible(),false);
    assert.equal(await page.getByRole('dialog').count(),0,'task details must never auto-open');
    assert((await page.locator('#plan-card').boundingBox()).height<60,'collapsed task card must leave room for timeline');
    if(process.env.WORKSPACE_CAPTURE_TASKS==='1')await page.locator('#plan-card').screenshot({path:path.join(__dirname,'../docs/images/dashboard-task-button.png')});
    await page.click('#task-details');await page.locator('.task-receipt.blocked').waitFor();
    ...
    assert.equal(await page.locator('.task-history').first().getAttribute('open'),null,'old execution records stay collapsed');
    await page.keyboard.press('Escape');assert.equal(await page.getByRole('dialog').count(),0);
    await update(complete.map(p=>({...p,evidence:''})),{task_state:'active'});
    await delay(1200);assert.equal(await page.getByRole('dialog').count(),0,'polling changes cannot reopen the panel');
    await page.click('#task-details');await page.locator('.task-receipt.verification_required').waitFor();
    assert.match(await page.locator('.task-receipt').innerText(),/不代表实际工作没有完成/);
    assert.equal(await page.getByRole('button',{name:'复制续做提示',exact:true}).count(),0);
    await page.getByRole('button',{name:'复制核对提示',exact:true}).waitFor();
    if(process.env.WORKSPACE_CAPTURE_TASKS==='1')await page.getByRole('dialog').screenshot({path:path.join(__dirname,'../docs/images/dashboard-task-details.png')});
    ...
    await page.setViewportSize({width:640,height:720});assert(await page.getByRole('dialog').evaluate(e=>e.getBoundingClientRect().right<=innerWidth));
    await page.emulateMedia({colorScheme:'dark'});await delay(250);
    if(process.env.WORKSPACE_CAPTURE_TASKS==='1')await page.getByRole('dialog').screenshot({path:path.join(__dirname,'../work/task-details-dark.png')});
    await page.keyboard.press('Escape');assert(await page.locator('#plan-card').evaluate(e=>e.scrollWidth<=e.clientWidth+1));
    await page.click('#plan-toggle');assert.equal(await page.locator('#plan-body .animate-spin').count(),0);assert.deepEqual(errors,[]);
