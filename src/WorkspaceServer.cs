...
{
    static JavaScriptSerializer Json { get { return new JavaScriptSerializer { MaxJsonLength = 8 * 1024 * 1024 }; } }
    static readonly Dictionary<string, Command> Commands = new Dictionary<string, Command>();
    static readonly UTF8Encoding Utf8 = new UTF8Encoding(false);
    [ThreadStatic] static string CurrentTool;
    ...
    static object Result(object value, bool error = false) { var command=value as Dictionary<string,object>;if(command!=null&&command.ContainsKey("error_code")&&command["error_code"]!=null)error=true;if(command!=null&&command.ContainsKey("exit_code")&&command["exit_code"]!=null&&!Convert.ToBoolean(command["stopped"]))error=Convert.ToInt32(command["exit_code"])!=0||Convert.ToBoolean(command["timed_out"]);return new { content = new[] { new { type = "text", text = value is string ? (string)value : Json.Serialize(value) } }, structuredContent=new{tool=CurrentTool,result=value,isError=error}, isError = error }; }
    static string Title(string name){string[] names={"get_workspace_status","list_commands","read_command","stop_command","git_status","git_diff","read_image","create_directory","list_directory","read_file","write_file","edit_file","exec_command","poll_command","show_changes","file_info","search_files","search_text","write_stdin","open_workspace","update_plan","apply_patch"};string[] titles={"连接与工具诊断","命令会话","查看命令进度","停止命令","Git 状态","Git 差异","查看图片","创建目录","浏览目录","阅读文件","写入文件","编辑文件","执行命令","读取命令输出","修改汇总","文件信息","搜索文件","搜索内容","发送命令输入","打开工作区","更新执行计划","应用文件补丁"};int i=Array.IndexOf(names,name);return name=="render_workspace"?"打开实时工作面板":name=="read_workspace_activity"?"读取工作区活动":i<0?name:titles[i];}
    ...
    static object ToolMeta(string name)
    {
        ...
        if(name=="render_workspace"){meta["ui"]=new{resourceUri=Presentation.LiveResource,visibility=new[]{"model","app"}};meta["openai/outputTemplate"]=Presentation.LiveResource;}
        else meta["ui"]=new{visibility=new[]{"model","app"}};
        return meta;
    }
    static object[] Tools()
    {
        return new[] {
            Tool("register_conversation","FIRST register this conversation before workspace work. Provide a clear title and workspace path; omit chat_id unless the actual ChatGPT /c/ UUID is known (never fabricate). Announce the returned title and local dashboard URL before proceeding. Include returned thread_id on EVERY tool call. Reuse the ID in this conversation; other chats must register separately. The independent local dashboard does not depend on ChatGPT cards.",false,new Dictionary<string,object>{{"title",Schema("string","Human-readable conversation/task title, 1..120 characters")},{"path",Schema("string","Absolute workspace directory")},{"chat_id",Schema("string","Actual known ChatGPT chat UUID, optional; never guess")}},"title","path"),
            Tool("render_workspace","Open the live workspace activity panel BEFORE starting multi-step work, after open_workspace. Return immediately, let the user see the panel, then continue work. The panel automatically observes subsequent tools, plans and commands in this MCP instance. Reuse it; do not render after every operation. Can be requested explicitly to inspect ongoing activity.",true,new Dictionary<string,object>{{"path",Schema("string","Absolute workspace directory to observe")}},"path"),
            Tool("read_workspace_activity","Read a non-blocking live snapshot of a workspace: running tool, recent activity, plan, bounded command output and UI connection diagnostics. This does not consume output, execute commands or change files. UI polls this tool; no template is attached.",true,new Dictionary<string,object>{{"path",Schema("string","Absolute workspace directory")},{"viewer_id",Schema("string","Optional UI instance identifier, max 80 characters")},{"bridge",new{type="string",@enum=new[]{"standard","legacy"}}},{"display_mode",new{type="string",@enum=new[]{"inline","pip","fullscreen","unknown"}}}},"path"),
            Tool("open_workspace","Open a workspace before coding: discover scoped AGENTS.override.md/AGENTS.md guidance, Git root, standalone skill paths, available shells and current plan. After reading guidance, call render_workspace with this path before multi-step work so the user can watch progress. Read nested guidance again when changing directory scope. Does not grant extra permissions.",true,new Dictionary<string,object>{{"path",Schema("string","Absolute working directory")}},"path"),
            Tool("update_plan","Publish concise execution steps and their actual status for this workspace. At most one step may be in_progress. Plans are process-local, not a scheduler or proof that work completed.",false,new Dictionary<string,object>{{"path",Schema("string","Absolute workspace directory")},{"explanation",Schema("string","Brief reason for the current plan update")},{"plan",new{type="array",minItems=1,maxItems=20,items=new{type="object",properties=new Dictionary<string,object>{{"step",Schema("string","Concrete step, maximum 240 characters")},{"status",new{type="string",@enum=new[]{"pending","in_progress","completed"}}}},required=new[]{"step","status"},additionalProperties=false}}}},"path","plan"),
    ...
    static object Call(string name, Dictionary<string,object> a)
    {
        ...
        if(name=="register_conversation"){
            ...
            Console.Error.WriteLine("[Workspace] [thread="+id+"] 对话登记 | "+S(a,"title").Replace("\n"," ").Replace("\r"," ")+" | "+LocalDashboard.Url+"#thread="+id);
            return Result(registration);
        }
        if(name=="render_workspace"||name=="read_workspace_activity")return Result(LiveSnapshot(a));
        if(name=="get_workspace_status")return Result(new{version=Version,default_shell=WorkspaceContext.DefaultShell,instance_id=InstanceId,dashboard_url=LocalDashboard.Url,conversations=WorkspaceThreads.List(),executable=Presentation.DisplayPath(System.Windows.Forms.Application.ExecutablePath),tool_count=Tools().Length,tools=Tools().Select(t=>t.GetType().GetProperty("name").GetValue(t,null)).ToArray(),running_commands=Commands.Count(x=>!x.Value.Process.HasExited),activity=WorkspaceActivity.Read(""),plans=WorkspaceContext.AllPlans(),ui=WorkspaceActivity.Diagnostics(""),scope="当前 MCP 进程；其他连接、过去进程和模型思考不可见。若工具缺失，请刷新宿主工具元数据并核对实际连接。"});
    ...
    static object RunCall(Dictionary<string,object> p,StreamWriter output)
    {
        ...
        CurrentThread=WorkspaceThreads.Validate(S(args,"thread_id"));
        ...
        Action<string> progress=message=>{lock(progressGate){if(!active||token==null)return;lock(OutputGate)output.WriteLine(new JavaScriptSerializer().Serialize(new{jsonrpc="2.0",method="notifications/progress",@params=new{progressToken=token,progress=step++,message=message}}));}};
        ...
        using(var timer=new System.Threading.Timer(_=>{lock(progressGate){if(!active)return;Console.Error.WriteLine(logPrefix+name+" | RUNNING | "+watch.ElapsedMilliseconds+" ms");progress(Title(name)+"仍在执行 · "+watch.Elapsed.TotalSeconds.ToString("F0")+" 秒");}},null,2000,2000))
        {
            ...
            try{result=Call(name,args);failed=(bool)result.GetType().GetProperty("isError").GetValue(result,null);if(failed)code="COMMAND_FAILED";}
    ...
    public static void Run()
    {
        ...
        using(var input=new StreamReader(Console.OpenStandardInput(),Utf8))using(var output=new StreamWriter(Console.OpenStandardOutput(),Utf8){AutoFlush=true}) {
            ...
            string line;try{while((line=input.ReadLine())!=null){object id=null;try{
                ...
                if(method=="initialize"){Console.Error.WriteLine("[Workspace] initialize | "+Version+" | instance="+InstanceId);result=new{protocolVersion="2025-06-18",capabilities=new{tools=new{listChanged=false},resources=new{listChanged=false}},serverInfo=new{name="local-workspace",version=Version},instructions="FIRST call register_conversation with a clear title and path; include actual chat_id only if known. Tell the user the title and returned dashboard_url before work. Pass returned thread_id on every tool call. Never attach another chat to a remembered global thread. Omitted calls appear as unassigned. The independent local browser dashboard is always available, even when ChatGPT hides cards. Windows local workspace with read, write, edit, search, native image reading, Git inspection and hidden Git Bash execution by default; explicitly select shell=powershell or shell=pwsh for PowerShell syntax. Begin with open_workspace to read scoped project guidance, discover shells and inspect the current plan. Then call render_workspace once BEFORE doing multi-step work, return its panel visibly to the user and continue working. Keep that panel open; it polls read_workspace_activity independently. Do not postpone rendering until the task is finished. Data tools intentionally do not create new cards. If no UI is visible, explicitly render_workspace and inspect its ui diagnostics rather than claiming the client cannot display UI. Use update_plan for multi-step work and apply_patch for reviewed multi-file changes. get_workspace_status verifies the actual server and available tools. Commands accept cmd/cwd/yield_time_ms; write_stdin continues sessions or sends input, and never re-runs the command. This is a local execution harness, not a model runtime or PTY. All local drives are available subject to OS permissions. Discover roots using list_directory with an empty path. Prefer returned forward-slash absolute paths; never insert Markdown escape backslashes before underscores. Inspect isError, exit_code and timed_out: an error is not success, a running session is not completion. Poll command sessions instead of re-running commands. read_command provides a non-consuming output snapshot for live UI; poll_command consumes only incremental output. Read AGENTS.md before changing a project. show_changes excludes shell/external changes; use git_status and git_diff for Git changes. Search partial/skip flags must not be interpreted as exhaustive results. Some hosts may hide app cards or filter tool discovery. Text results remain authoritative. Never claim the server is read-only merely because a host omits tools; report the visible tool names and try get_workspace_status when available. Report actual work progress and concrete failures to the user; do not claim changes without tool receipts."};}
                ...
                else if(method=="resources/list")result=Presentation.ResourceList();
                else if(method=="resources/read"){string uri=S((Dictionary<string,object>)req["params"],"uri");result=Presentation.ReadResource(uri);WorkspaceActivity.ResourceRead(uri);}
                else if(method=="resources/templates/list")result=new{resourceTemplates=new object[0]};
                else if(method=="tools/call"){
                    var call=(Dictionary<string,object>)req["params"];string tool=S(call,"name");
                    if(tool=="read_workspace_activity"||tool=="render_workspace"){try{result=Call(tool,call.ContainsKey("arguments")?(Dictionary<string,object>)call["arguments"]:new Dictionary<string,object>());}catch(Exception ex){CurrentTool=tool;result=Result(new{error_code="TOOL_ERROR",message=ex.Message},true);}}
