...
static class WorkspaceServer
{
    ...
    static readonly string Instructions="Use open_workspace before editing to read project guidance. Calls are grouped automatically when host session metadata is present; otherwise register_conversation and pass thread_id. Use absolute Windows paths. Continue running commands with write_stdin; never rerun them to poll. Check isError, exit_code and timed_out. Full data is in structuredContent.result; content is a summary. Import chat attachments with import_file into a new path. The local dashboard shows activity; no chat widget is required.";
    ...
    static object Result(object value, bool error = false) { var command=value as Dictionary<string,object>;if(command!=null&&command.ContainsKey("error_code")&&command["error_code"]!=null)error=true;if(command!=null&&command.ContainsKey("exit_code")&&command["exit_code"]!=null&&!Convert.ToBoolean(command["stopped"]))error=Convert.ToInt32(command["exit_code"])!=0||Convert.ToBoolean(command["timed_out"]);return new { content = new[] { new { type = "text", text = WorkspaceContracts.Summary(CurrentTool,value,error) } }, structuredContent=new{tool=CurrentTool,result=value,isError=error,thread_id=CurrentThread??"unassigned"}, isError = error }; }
    static string Title(string name){string[] names={"get_workspace_status","list_commands","read_command","stop_command","git_status","git_diff","read_image","create_directory","list_directory","read_file","write_file","edit_file","exec_command","poll_command","show_changes","file_info","search_files","search_text","write_stdin","open_workspace","update_plan","apply_patch"};string[] titles={"连接与工具诊断","命令会话","查看命令进度","停止命令","Git 状态","Git 差异","查看图片","创建目录","浏览目录","阅读文件","写入文件","编辑文件","执行命令","读取命令输出","修改汇总","文件信息","搜索文件","搜索内容","发送命令输入","打开工作区","更新执行计划","应用文件补丁"};int i=Array.IndexOf(names,name);return name=="import_file"?"接收聊天附件":name=="read_workspace_activity"?"读取工作区活动":i<0?name:titles[i];}
